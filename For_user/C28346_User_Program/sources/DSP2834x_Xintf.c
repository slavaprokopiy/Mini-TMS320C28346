...
void InitXintf(void)
{
    ...
    // All Zones---------------------------------
    // Timing for all zones based on XTIMCLK = SYSCLKOUT/4 = 300 МГц/4 = 75 МГц (у PSRAM max частота работы 133 МГц)
    EALLOW;
	// XTIMCLK = SYSCLKOUT/2
    ...
    // Lead and Trail (Write) must always be 1 or greater
    // Zone write timing
    XintfRegs.XTIMING7.bit.XWRLEAD = 1;//3;
...
void InitXintf32Gpio()
{
     ...
     GpioCtrlRegs.GPAMUX1.bit.GPIO0 = 0; // CRE
     GpioCtrlRegs.GPADIR.bit.GPIO0	= 1;
     GpioCtrlRegs.GPAPUD.bit.GPIO0	= 0;
     GpioDataRegs.GPACLEAR.bit.GPIO0 = 1;

     GpioCtrlRegs.GPAMUX1.bit.GPIO2 = 0; // ADV
...
//XINTF block================================================================
//!#define XintfSize  (XintffuncsLoadEnd - XintffuncsLoadStart)// Размер внешней памяти в количестве 32-битных слов

/*#pragma DATA_SECTION(xintf_data,"ZONE7DATA");
int xintf_data[XintfSize];	// Данные во внешней памяти
int xintf_out_data[XintfSize];	// Массив данных для передачи данных во внешнюю память
int xintf_in_data[XintfSize];		// Массив для приема данных из внешней памяти
*/

void XintfWrite(void){
	/*
	int i;


	for(i=0;i < XintfSize;i++){
		xintf_out_data[i] = i;
	}
	for(i=0;i < XintfSize;i++){
		xintf_data[i] = xintf_out_data[i];
	}*/

	/*Uint32 *load_addr;
	load_addr = XintffuncsLoadStart;

	Uint32 *run_addr;
	run_addr = XintffuncsRunStart;

	while(load_addr < XintffuncsLoadEnd){
		*run_addr = *load_addr;
				run_addr++;
				load_addr++;
	}*/
...
void XintfRead(void){
	/*	int i;
	for(i =0;i < XintfSize;i++){
		xintf_in_data[i] = xintf_data[i];
		if(xintf_in_data[i]!=xintf_out_data[i]){;}
	}*/
}
//===========================================================================
// No more.
